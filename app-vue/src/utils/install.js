...
export default {
  install(Vue, options) {
     ...
     * @param {children: string} props   参数
     * @returns 
     */
    Vue.prototype.$_removeTreeEmpty = (treeList, props = { children: 'children' }) => { // 去除树结构空子节点
      function removeFn(data) {
        data.forEach((item, index, arr) => {
          if (item[props.children] && item[props.children].length > 0) {
            arr[index][props.children] = removeFn(item[props.children])
          } else {
            delete arr[index][props.children]
          }
        })
        return data
      }
      return removeFn(treeList)
    }
  }
}
